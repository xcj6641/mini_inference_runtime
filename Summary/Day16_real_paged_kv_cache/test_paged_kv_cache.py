...
def test_prefill_writes_real_kv_into_paged_cache(
    ...
    # FakeRunner prefill generated a physical KV cache
    # with batch_size=1 and sequence_length=4.
    expected_batched_cache = (
        fake_runner._make_batched_kv_cache(
            batch_size=1,
            sequence_length=4,
        )
    )
...
def test_prefill_paged_cache_excludes_left_padding(
    ...
    # Persistent paged KV stores only logical tokens.
    assert paged_a[0][0].shape[2] == 3
    assert paged_a[0][1].shape[2] == 3

    assert paged_b[0][0].shape[2] == 6
    assert paged_b[0][1].shape[2] == 6

    # FakeRunner saw a padded batch of physical length 6.
    expected_batched_cache = (
        fake_runner._make_batched_kv_cache(
            batch_size=2,
            sequence_length=6,
    ...
    expected_a_physical = (
        expected_per_request[0]
    )

    expected_b_physical = (
        expected_per_request[1]
    )

    # A:
    # physical = [PAD PAD PAD A0 A1 A2]
    # logical  = [A0 A1 A2]
    source_start_a = 6 - 3
    ...
    for layer_idx in range(
        len(expected_a_physical)
    ):
        (
            expected_key_a_physical,
            expected_value_a_physical,
        ) = expected_a_physical[layer_idx]
    ...
    for layer_idx in range(
        len(expected_b_physical)
    ):
        (
            expected_key_b,
            expected_value_b,
        ) = expected_b_physical[layer_idx]
...
def test_freed_paged_kv_blocks_can_be_reused_without_stale_data() -> None:
    block_manager = KVBlockManager(
        num_blocks=1,
        block_size=4,
    )
